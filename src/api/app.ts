        ...
        app.get('/api/fragments/hosts', async (_request, reply) => {
            ...
            const cards = hosts.map((host) => {
                ...
                const runtimeRows = hostRuntimes.map((runtime) => {
                    const action = runtime.state === 'shutdown' ? 'boot' : 'shutdown';
                    const label = runtime.state === 'shutdown' ? 'Boot' : 'Stop';
                    ...
                    const stateClass = runtime.state === 'booted' ? 'ready' : runtime.state === 'booting' ? 'connecting' : 'unavailable';
                ...
                return `<article class="host-card${online ? degraded ? ' degraded' : '' : ' offline'}"><div class="host-card-head"><div><span class="eyebrow">Execution host</span><h3>${escapeHtml(host.id)}</h3><p>${escapeHtml(host.hostname)} · ${escapeHtml(host.os)} ${escapeHtml(host.arch)}</p></div>${status}</div>${metricHtml}<div class="connection-chips">${capabilities || '<span class="connection-chip unavailable">capabilities unavailable</span>'}</div>${error}${hostRuntimes.length ? `<div class="host-runtime-list"><div class="host-runtime-head"><strong>Virtual runtimes</strong><span>${hostRuntimes.filter(({ state }) => state !== 'shutdown').length}/${hostRuntimes.length} running</span></div>${runtimeRows}</div>` : empty}</article>`;
            }).join('');
            const healthyHosts = hosts.filter((host) => host.online !== false && !host.error).length;
